...
/// Experimental ring-backed counterpart to `Destination`.
///
/// Buffered bytes can wrap physically; a vector-capable backend receives both readable
/// regions in logical order, avoiding compaction while preserving ordinary backpressure
/// and short-drain semantics.
package struct RingDestination<Backend: DestinationBackend & ~Copyable>: ~Copyable {
    ...
    package var statistics: DestinationStatistics {
        borrowing get {
            .init(
    ...
    package mutating func flush() throws -> DestinationFlush {
    ...
    private mutating func drainBufferedOnce() throws -> DestinationDrain {
        ...
        let result = try buffer.withReadableRegions { first, second in
            ...
            return try backend.drain(first: first, second: second)
        }
    ...
    private mutating func drainDirect(
        ...
        let result = try backend.drain(first: bytes, second: empty)
    ...
    private func unavailableWrite(
        ...
        return .partial(.knownPositive(accepted))
    }
